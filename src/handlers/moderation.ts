...
async function requireAdmin(
  ...
  try {
    ...
    logger.error("Error checking admin status", err, { userId });
    return c.json(
...
moderation.post("/users/:handle/ban", requireAuth, requireAdmin, async (c) => {
  ...
  if (body.reason.trim().length > LIMITS.MAX_MODERATION_REASON_LENGTH) {
    return c.json(
      {
        success: false,
        error: `Ban reason must be ${LIMITS.MAX_MODERATION_REASON_LENGTH} characters or less`,
      },
      400,
    );
  }
...
moderation.post("/posts/:id/takedown", requireAuth, requireAdmin, async (c) => {
  ...
  if (body.reason && body.reason.length > LIMITS.MAX_MODERATION_REASON_LENGTH) {
    return c.json(
      {
        success: false,
        error: `Takedown reason must be ${LIMITS.MAX_MODERATION_REASON_LENGTH} characters or less`,
      },
      400,
    );
  }
...
moderation.post(
  ...
  async (c) => {
    ...
    const action = body.isAdmin
      ? "granted admin privileges"
      : "had admin privileges revoked";
