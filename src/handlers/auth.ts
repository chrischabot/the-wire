...
auth.post("/signup", rateLimit(RATE_LIMITS.signup), async (c) => {
  ...
  if (!handleResult.valid) {
    log.warn("Handle validation failed", {
      handle: body.handle,
  ...
  try {
    ...
    if (!initResp.ok) {
      ...
      log.error("UserDO.initialize failed", new Error(errText), {
        userId,
    ...
    const addFollowerResp = await stub.fetch(
      "https://do.internal/add-follower",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId }),
    ...
    if (!addFollowerResp.ok) {
      log.warn("Self add-follower failed", {
        userId,
        status: addFollowerResp.status,
      });
    }
    ...
    if (handle !== FOUNDER_HANDLE) {
      ...
      try {
        ...
        if (founderUserId) {
          ...
          log.info("Auto-followed founder account", {
        ...
        try {
          ...
          try {
            ...
            if (userPostsIndex) {
              ...
              log.debug("Backfilled posts from seed user", {
                seedHandle,
                addedCount,
              });
            }
          } catch (backfillErr) {
            log.warn("Failed to backfill posts from seed user", {
              seedHandle,
              error: backfillErr,
            });
          }
          ...
          log.warn("Failed to auto-follow seed user", {
            seedHandle,
            error: followErr,
          });
        }
    ...
    const response: AuthResponse = {
      user: { id: userId, email, handle, avatarUrl: "", displayName: handle },
      token,
      expiresAt,
    };
...
auth.post(
  ...
  async (c) => {
    ...
    try {
      ...
      if (!userData) {
        log.error("User data not found for existing email mapping", null, {
          email,
          userId,
        });
        return unauthorized("Invalid credentials");
      ...
      const doId = c.env.USER_DO.idFromName(authUser.id);
      const stub = c.env.USER_DO.get(doId);
      const profileResp = await stub.fetch("https://do.internal/profile");
      const profile =
        (await profileResp.json()) as import("../types/user").UserProfile;
...
auth.post(
  "/reset/request",
  rateLimit({ limit: 3, windowSeconds: 3600, keyPrefix: "rl:reset" }),
  async (c) => {
...
auth.post(
  "/reset/confirm",
  rateLimit({ limit: 5, windowSeconds: 3600, keyPrefix: "rl:reset-confirm" }),
  async (c) => {
